...
import (
	...
	birdbgp "k8s.io/sample-controller/pkg/controllers/birdbgp"
...
func main() {
	...
	controllerConfig := birdbgp.ControllerConfig{
		Nodename:        nodename,
		...
		BirdConfigDir:   birdConfigDir,
		BirdBGPInformer: customInformerFactory.Networking().V1alpha1().BirdBGPProtocols(),
	}
	controller := birdbgp.NewController(ctx, controllerConfig)
...
func init() {
	...
	flag.StringVar(&birdSocketPath, "bird-socket-path", "/var/run/bird/bird.ctl", "The path to the bird socket.")
	flag.StringVar(&birdConfigDir, "bird-config-dir", "/etc/bird/conf.d", "The directory where the bird BGP protocol configuration files are stored.")
}
